...
@pytest.fixture
def portal_config():
    return PortalConfiguration.objects.create(
...
def test_complete_signup_creates_integration_config(monkeypatch, api_client, user, portal_config, tenant_config):
    ...
    monkeypatch.setattr("portal.integrations.views.requests.get", fake_get)
    monkeypatch.setattr("portal.integrations.views.requests.post", fake_post)
