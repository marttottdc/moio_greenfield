...
class MediaStore:
    def __init__(
        ...
        *,
        ...
        use_django_storage: bool = False,
    ) -> None:
        self.workspace_root = workspace_root.expanduser().resolve()
        self.tenant_schema = _safe_component(tenant_schema, "public")
        self.workspace_slug = _safe_component(workspace_slug, "main")
        self.log = logger
        self.use_django_storage = bool(use_django_storage)
        if self.use_django_storage:
            self.local_root = Path("")  # not used
        else:
    ...
    def _django_storage_key(self, relative_path: str) -> str:
        """Path under Django media: agent_console/{tenant}/{workspace}/{session}/{run}/{category}/{file}"""
        return f"agent_console/{self.tenant_schema}/{self.workspace_slug}/{relative_path}"
    ...
    async def store_bytes(
        ...
        *,
        ...
        if self.use_django_storage:
            def _save_django(storage_key: str, data: bytes) -> None:
                from django.core.files.base import ContentFile
                from django.core.files.storage import default_storage
                default_storage.save(storage_key, ContentFile(data))

            storage_key = self._django_storage_key(relative_path)
            await asyncio.to_thread(_save_django, storage_key, payload)
            from django.core.files.storage import default_storage
            download_url = await asyncio.to_thread(default_storage.url, storage_key)
