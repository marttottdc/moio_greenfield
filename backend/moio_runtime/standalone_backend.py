...
class StandaloneAgentBackend:
    ...
    def __init__(
        ...
        *,
        ...
        self.config = config
        self.log = logger or logging.getLogger("moio_runtime.backend")
    ...
    async def agent_runtime(
        ...
        *,
        ...
        effective_tool_allowlist = self.effective_tool_allowlist(
            initiator=initiator,
    ...
    def effective_tool_allowlist(
        ...
        *,
        ...
        return self._effective_runtime_tool_allowlist(
            requested,
            initiator=initiator,
            profile_allowlist=profile_allowlist,
        )

    def _resolved_user_allowed_plugins(self, initiator: dict[str, Any] | None = None) -> set[str]:
        if callable(self.plugin_user_allowlist_resolver):
            try:
                resolved = self.plugin_user_allowlist_resolver(initiator if isinstance(initiator, dict) else None)
            except TypeError:
                resolved = self.plugin_user_allowlist_resolver(None)
            except Exception as exc:
    ...
    async def resources(
        ...
        *,
        ...
        user_allowed_plugins = self._resolved_user_allowed_plugins(initiator)
        active_plugins = [
            plugin
    ...
    async def _execute_run(
        ...
        *,
        ...
        if attachment_names:
            ...
            path_lines: list[str] = []
            for item in attachment_context["items"][:8]:
                ...
                if not path:
                    continue
                if name:
                    path_lines.append(f"- {name}: {path}")
                else:
                    path_lines.append(f"- {path}")
                if download_url:
                    path_lines.append(f"  download: {download_url}")
            if path_lines:
                user_text_for_session += "\n[Attachment paths]\n" + "\n".join(path_lines)
    ...
    async def _prepare_attachments_context(
        ...
        *,
        ...
        lines = [
            "Current user turn includes uploaded attachments. Use this extracted context and the saved file paths.",
            "You may call resource.read on these paths for deeper inspection.",
        ]
    ...
    def _extract_history_items(history: list[dict[str, Any]]) -> list[dict[str, str]]:
        ...
        for item in history:
            ...
            text = StandaloneAgentBackend._append_message_artifact_hints(item, text)
            if not text:
                continue
            context_scope = normalize_session_context_scope(item.get("contextScope"))
            if context_scope == "personal":
                owner_label = StandaloneAgentBackend._history_owner_label(item)
                prefix = f"[Private context for {owner_label}]" if owner_label else "[Private context]"
                text = f"{prefix}\n{text}"
            elif role == "user":
                author_label = StandaloneAgentBackend._history_author_label(item)
                if author_label:
    ...
    async def _run_steps(
        ...
        *,
        ...
        runtime_allowlist = self._effective_runtime_tool_allowlist(tool_allowlist, initiator=initiator)
    ...
    def _effective_runtime_tool_allowlist(
        self,
        ...
        *,
        ...
        allowed_plugins = self._resolved_user_allowed_plugins(initiator)
        ...
        if isinstance(value, dict):
            ...
            for key, item in value.items():
                ...
                if key_l in {"password", "passphrase", "secret", "token", "access", "refresh", "api_key", "authorization"}:
                    ...
                    redacted[str(key)] = StandaloneAgentBackend._sanitize_for_logs(item)
            return redacted
        if isinstance(value, list):
            return [StandaloneAgentBackend._sanitize_for_logs(item) for item in value]
        return value
    ...
    def _redact_tool_args_for_event(tool_name: str, raw_arguments: str) -> str:
        ...
        safe_payload = StandaloneAgentBackend._sanitize_for_logs(payload)
